...
const Tags = ({ setAmount }) => {
    ...
    return (
        <div className='w-full flex gap-2 flex-wrap break-all'>
            ...
            {tags?.map((string, i) =>
                <div key={i} onClick={() => handleClic(string)} className={` cursor-pointer h-[32px] font-semibold text-xs rounded-md flex gap-1 items-center ${button === string ? 'bg-slate-700 text-white shadow-md shadow-gray-600' : 'bg-slate-400 shadow-md shadow-gray-300'}`}>
                    ...
                    {button === string ?
                        <>
                            ...
                            <Separator orientation="vertical" className='h-5 font-bold' />
                            <DropdownMenu asChild>
                                <DropdownMenuTrigger>
                                    <img className='h-6' src={moremenu} alt="" />
                                </DropdownMenuTrigger>
