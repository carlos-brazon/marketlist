...
const Tags = () => {
  ...
  return (
    <div className='flex gap-2 justify-center'>
        ...
        {tags.map(string => <button onClick={() => setButton(tags.length==1 ? 'Compras' : string) } className={`p-1 font-semibold text-base rounded-md  ${button === string ? 'bg-blue-800 text-white hover:shadow-blue-800 shadow-md shadow-blue-950' : 'bg-blue-400 hover:shadow-blue-400 shadow-md shadow-gray-500'}`}>{string}</button>)}
    </div>
  )
}
