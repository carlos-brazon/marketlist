...
const MercadonaDialog = () => {
  ...
  const getAllItems = async () => {
    ...
    try {
      ...
      // 2️⃣ Obtener productos de cada categoría usando nuestra API Route
      const allData = await Promise.all(
        ids.map(id =>
      ...
      // 3️⃣ Acumular todos los productos
      const allProducts = filteredData.flatMap(category => {
        let products = category.products ? [...category.products] : [];
        if (category.categories) {
          category.categories.forEach(sub => {
      ...
      localStorage.setItem('allItemsMercadona2', JSON.stringify(allProducts));
