    ...
    return (
        <div className=' flex flex-col items-center p-2 w-fit gap-2'>
            ...
            <header className="flex items-center relative gap-10 p-4 text-white">
                <Link to={'/'} className={`py-3 px-6 font-semibold text-sm leading-4 rounded bg-slate-400 text-white`}>{'Home'} </Link>
                ...
                {!userIn ? <button className={`${!userIn ? 'hidden' : ''}`}>Log In</button> : <button className={`bg-slate-400 text-white py-3 px-6 font-semibold text-sm leading-4 rounded ${!userIn ? '' : ''}`} onClick={() => signOut(auth)}>Log Out</button>}
            </header>
